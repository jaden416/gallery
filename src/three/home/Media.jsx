...
export default function Media({
  ...
  useEffect(() =>{
    ...
    if(focus.state === false)
      reset()

    if (focus.index === index)
      show()
    
    if(focus.index !== index && focus.index != null)
      hide()
  ...
  function show(){
    ...
    tl.to(animation.current, { size: 1, position: 1, duration: .35, ease: "power3", })
  ...
  function hide(){
    ...
    gsap.to(animation.current, { position: 0, duration: .35, ease: "power3"})
  ...
  function reset(){
    ...
    if(animation.current.position === 1 )
      gsap.to(text, { alpha: 0, y: 0})

    gsap.to(opacity.current, { current: 1, duration: .35, ease: "power3"})
    gsap.to(animation.current, { position: .5, size: 0, duration: .35, ease: "power3"})
    ...
    const positionY = 
      gsap.utils.interpolate(
        [
          ...
          findY(viewport.height, mesh.current.scale.y, bounds.current.top, size.height, extra.current.y, stagger, y), // initial
